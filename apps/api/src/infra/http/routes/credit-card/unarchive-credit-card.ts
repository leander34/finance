...
export async function unarchiveCreditCard(app: FastifyInstance) {
  app
    .withTypeProvider<ZodTypeProvider>()
    .register(auth)
    .register(getUserDeviceInfo)
    .patch(
      '/organizations/:slug/credit-cards/:id/unarchive',
      {
        schema: {
          tags: ['Credit Cards'],
          summary: 'Unarchive a credit card',
          security: [
            {
              bearerAuth: [],
            },
      ...
      async (request, reply) => {
        ...
        if (!creditCard.archivedAt) {
          throw new BadRequestError(
            'pt-br.credit-card.credit-card-already-unarchive',
          )
        }
